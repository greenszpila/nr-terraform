...
data "newrelic_entity" "app_name" {
  ...
  name = (module.global_vars.nr_appname) // modify the variable in variables.tf to match with your appName
  type = "APPLICATION"
  domain = "APM"
}
...
resource "newrelic_one_dashboard" "dashboard_name" {
  ...
  page {
    ...
    widget_line {
      title = "Golden Signals - Latency - Relicstaurants - Line"
      row = 1
      column = 5
      width = 4
      height = 3
      ...
      nrql_query {
        query = "SELECT average(apm.service.overview.web) * 1000 as 'Latency' FROM Metric WHERE appName = '${data.newrelic_entity.app_name.name}' since 30 minutes ago TIMESERIES AUTO"
      }
    }

    widget_stacked_bar {
      title = "Golden Signals - Latency - Relicstaurants - Stacked Bar"
      row = 1
      column = 9
      width = 4
      height = 3
    ...
    widget_area {
      title = "Golden Signals - Errors - Relicstaurants - Area"
      row = 4
      column = 5
      width = 4
      height = 3
      ...
      nrql_query {
        query = "SELECT (count(apm.service.error.count) / count(apm.service.transaction.duration))*100 as 'Errors' FROM Metric WHERE (appName = '${data.newrelic_entity.app_name.name}') AND (transactionType = 'Web') SINCE 30 minutes ago TIMESERIES AUTO"
      }
    }

    widget_billboard {
      title = "Golden Signals - Errors - Relicstaurants - Billboard Compare With"
      row = 4
      column = 9
      width = 4
      height = 3
      ...
      nrql_query {
        query = "SELECT (count(apm.service.error.count) / count(apm.service.transaction.duration))*100 as 'Errors' FROM Metric WHERE (appName = '${data.newrelic_entity.app_name.name}') AND (transactionType = 'Web') SINCE 30 minutes ago COMPARE WITH 30 minutes ago"
      }
    }
    ...
    widget_table {
      title = "Golden Signals - Traffic - Relicstaurants - Table"
      row = 7
      column = 5
      width = 4
      height = 3
      ...
      nrql_query {
        query = "SELECT rate(count(apm.service.transaction.duration), 1 minute) as 'Traffic' FROM Metric WHERE (appName = '${data.newrelic_entity.app_name.name}') AND (transactionType = 'Web') FACET path SINCE 30 minutes ago"
      }
    }

    widget_pie {
      title = "Golden Signals - Traffic - Relicstaurants - Pie"
      row = 7
      column = 9
      width = 4
      height = 3
    ...
    widget_line {
      ...
      nrql_query {
        query = "SELECT rate(sum(apm.service.cpu.usertime.utilization), 1 second) * 100 as 'cpuUsed' FROM Metric WHERE appName = '${data.newrelic_entity.app_name.name}' SINCE 30 minutes ago TIMESERIES AUTO"
      }

      nrql_query {
        query = "SELECT average(apm.service.memory.physical) * rate(count(apm.service.instance.count), 1 minute) / 1000 as 'memoryUsed %' FROM Metric WHERE appName = '${data.newrelic_entity.app_name.name}' SINCE 30 minutes ago TIMESERIES AUTO"
      }
    }
    ...
    widget_line {
      ...
      nrql_query {
        query = "SELECT average(apm.service.memory.physical) * rate(count(apm.service.instance.count), 1 minute) / 1000 as 'memoryUsed %' FROM Metric WHERE appName = '${data.newrelic_entity.app_name.name}' SINCE 30 minutes ago COMPARE WITH 20 minutes ago TIMESERIES AUTO"
      }
    }
  }
}